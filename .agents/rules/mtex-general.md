---
name: mtex-general
description: Core development rules and subsystem guidelines for the MTEX toolbox
trigger: always_on
---
...
> `GEMINI.local.md` at the repo root, not here. Point `MATLAB_ROOT` at your install rather
...
`geometry/@vector3d/angle.m`. Each folder below has its own `GEMINI.md` with the detail.

- `geometry/` — vectors, orientations, Miller indices, symmetries, reference frames (see `geometry/GEMINI.md`)
- `EBSDAnalysis/` — EBSD data, grains, grain boundaries, reconstruction (see `EBSDAnalysis/GEMINI.md`)
- `PoleFigureAnalysis/` — pole figure data and ODF-from-pole-figure solvers
- `SO3Fun/`, `S2Fun/`, `S1Fun/` — function spaces on SO(3), the sphere, the circle (see `SO3Fun/GEMINI.md`, `S2Fun/GEMINI.md`, `S1Fun/GEMINI.md`)
- `TensorAnalysis/` — elastic/plastic tensor calculations (see `TensorAnalysis/GEMINI.md`)
- `plotting/` — all figure and plot code (see `plotting/GEMINI.md`)
- `interfaces/` — file format import/export (see `interfaces/GEMINI.md`)
- `doc/` — published example scripts, and the source of the online documentation (see `doc/GEMINI.md`)
...
**Read `tests/GEMINI.md` before adding a test.** It has the tier budgets, which file owns
