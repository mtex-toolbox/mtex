...
% a selection from a grid is a list that keeps the lattice, a block of
% subscripts stays a grid
isGrid = isa(ebsd,'EBSDgrid') || isa(ebsd,'EBSD3square');
toList = isGrid && ~check_option(varargin,'keepGrid') && ...
  (islogical(ind) || min(size(ind))==1);

if toList && isa(ebsd,'EBSD3square')
  ebsd = EBSD3(ebsd);
elseif toList
  ebsd = EBSD(ebsd);
elseif isGrid
