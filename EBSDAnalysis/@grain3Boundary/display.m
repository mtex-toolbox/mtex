...
displayClass(gB,inputname(1),'moreInfo',...
  referenceFrame.headerChar(gB.frame,gB.how2plot,gB.allV.how2plotPrivate));
...
for ip = 1:numel(ph2)
  ...
  ind = phId(:,1) == ph1(ip) & phId(:,2) == ph2(ip);
  num(ip) = sum(ind);
  ...
  % store area
  matrix{ip,4} = [xnum2str(100*sum(allArea(ind))/sum(allArea)) '%'];

end
matrix(num==0,:) = [];

cprintf(matrix,'-L',' ','-Lc',...
  {'faces' 'mineral 1' 'mineral 2' 'area'},'-d','  ','-ic',true);
