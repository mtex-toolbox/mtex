function job = clusterHyperGraph3(job,varargin)
...
if check_option(varargin,'finishMerged')
    ...
    zz(zz(:,1) == zz(:,2),:) = [];
    zz = unique(sort(zz,2),'rows');
    job.votes.parentId(:,1) = zz(sub2ind(size(zz),votes_spare.parentId(:,1),job.votes.parentId(:,1)));
    job.votes.parentId(:,2) = zz(sub2ind(size(zz),votes_spare.parentId(:,2),job.votes.parentId(:,2)));
