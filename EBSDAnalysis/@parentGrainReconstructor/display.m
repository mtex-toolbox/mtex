...
if ~isempty(job.votes)
  disp(' ');
  omega = 100*job.votes.prob(:,1);
  disp([' votes: ' int2str(size(job.votes,1)) ...
    ' ' ] );
  disp(['   probabilities: ' xnum2str(quantile(omega,0.8)) '%' ...
    ', ' xnum2str(quantile(omega,0.6)) '%' ...
    ', ' xnum2str(quantile(omega,0.4)) '%' ...
    ', ' xnum2str(quantile(omega,0.2)) '%' ...
    ' (quintiles)']);
