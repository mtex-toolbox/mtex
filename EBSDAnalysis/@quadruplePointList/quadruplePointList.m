classdef quadruplePointList < phaseList & dynProp
  
  % properties with as many rows as data
  properties
    id = zeros(0,1)      % indices of the vertices in grains.V
    V = zeros(0,2)       % vertices x,y coordinates
    grainId = zeros(0,4) % id's of the neigbouring grains to a face
    boundaryId = zeros(0,4)  % id's of the neigbouring ebsd data to a face    
  end
  ...
  methods
    function tP = quadruplePointList(id,V,grainId,boundaryId,phaseId,phaseMap,CSList)
