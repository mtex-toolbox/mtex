...
if check_option(varargin,'xcfBackend')
    warning('trueEbsd:xcfBackendGone', ...
...
if nImg < 2
    error('trueEbsd:tooFewImages','calcDistortion needs at least two maps');
end
if any(arrayfun(@(d) isempty(d.img),job.resizedList))
    error('trueEbsd:noPositions', ...
        'job.resizedList is empty - run pixelSizeMatch first');
end
if numel(job.T) ~= nImg-1
    error('trueEbsd:modelCount', ...
...
for n = 1:numel(job.resizedList)-1
    ...
    error('trueEbsd:orientationMismatch', ...
