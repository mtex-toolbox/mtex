...
assert(all(idx >= 1 & idx <= numel(job.imgList)),'trueEbsd:badIndex',...
  'This job has %d maps, asked for %s.',numel(job.imgList),mat2str(idx));

known = fieldnames(job.opt);

assert(mod(numel(varargin),2) == 0,'trueEbsd:badOptions',...
...
for k = 1:2:numel(varargin)
  ...
  assert(~isempty(hit),'trueEbsd:unknownOption',...
...
switch name
  ...
  case {'roiSize','edgeWidth'}
    if ischar(value) || isstring(value)
      ...
      assert(strcmp(value,'auto'),'trueEbsd:badOption',...
      ...
      assert(all(value >= 0),'trueEbsd:badOption',...
  ...
  case 'registerOn'
    value = lower(char(value));
    assert(any(strcmp(value,{'edge','raw'})),'trueEbsd:badOption',...
