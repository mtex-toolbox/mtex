classdef halfQuadraticFilter < EBSDFilter
  ...
  properties
    ...
    weight = @(t,eps) 1./(t.^2+e^2).^(1/2); % weight function handle b^*(t,epsilon)
  ...
  methods
    ...
    function q = smooth(F,q)           
      ...
      while max(max(M.dist(u_old,u)))>F.tol || entrance
        ...
        entrance = false;
        i = i+1;
        wx = F.weight(M.dist(u,ux),epsilon);
        wy = F.weight(M.dist(u,uy),epsilon);
        wxd =F.weight(M.dist(u,uxd),epsilon);
        wyd =F.weight(M.dist(u,uyd),epsilon);
        %Nullen streichen
        wx(maskx_zero)=0;
        wy(masky_zero)=0;
        wyd(maskyd_zero)=0;
        ...
        f(mask)=u(mask);
        %berechne gradient
        grad_u = M.log(u,f) + F.alpha(1)*wx.*M.log(u,ux) + ...
