...
function issueWarnings(warnings, S2F)
  if warnings.rangeTooFew
    warning(['Some fixed-radius neighborhoods had at most the ansatz dimension ' ...
      'many nodes and were replaced by minimal KNN neighborhoods.']);
  end

  if warnings.rangeTooMany
    warning(['Some fixed-radius neighborhoods exceeded the maximal ' ...
      'oversampling factor and were replaced by capped KNN neighborhoods.']);
  end

  if warnings.smoothTooFew
    warning(['At some centers the smooth support contained fewer than %d ' ...
      'positive-weight neighbors and was enlarged locally.'], S2F.dim);
  end

  if warnings.smoothAllCandidates
    warning(['At some centers all fetched KNN candidates had positive weight. ' ...
      'The smooth compact support may be truncated; fetch more candidates.']);
  end
end
