...
if S2F.subsample
  dist = angle(v.subSet(center_id), S2F.nodes.subSet(grid_id));
...
if ~S2F.centered
  ...
  g_book = permute(eval_basis_functions(S2F, v), [2, 3, 1]);
...
G_book = permute(reshape(G, nn, N, S2F.dim), [1, 3, 2]);
clear G center_id zloc;
...
if wantInfo
  geometryScore = local_geometry_score_S2( ...
    reshape(xloc, nn, N), reshape(yloc, nn, N), weights.');
end

clear ind weights dist deltas xloc yloc;
