function f = eval2v32(SO3F,ori,varargin)
...
% respect to j, we can use this to speed up computation
  ...
  for n=0:N
    ...
    Fhat=reshape(SO3F.fhat(deg2dim(n)+1:deg2dim(n+1)),2*n+1,2*n+1); Fhat=Fhat(:,1:n+1);

    d = Wigner_D(n,pi/2); d = d(:,1:n+1);
    D = permute(d,[1,3,2]) .* permute(d(1:n+1,:),[3,1,2]) .* Fhat;
  
    ghat(N+2+(-n:n),N+2+(-n:0),N+2+(-n:0)) = ghat(N+2+(-n:n),N+2+(-n:0),N+2+(-n:0)) + D;
  end
  ...
  ghat(2:end,N+2+(1:N),2:end) =  conj(flip(flip(flip(ghat(2:end,N+2+(-N:-1),2:end),1),2),3));
