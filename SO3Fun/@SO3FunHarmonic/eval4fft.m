function f = eval4fft(SO3F,ori,varargin)
...
%fft
fgrid=permute(fftn(ghat),[3,2,1]);
fgrid=fgrid(G(:,2)<pi).*exp(i*N*sum(Grid,2));

% evaluate SO3F at ori
a=Euler(ori,'nfft');
b=mod(round(a/(2*pi)*(2*N+1)),2*N+1);
ind=(N+1)*(2*N+1)*b(:,1)+(2*N+1)*b(:,2)+b(:,3)+1;
f=fgrid(ind);
