...
static void L2_normalized_sphericalHarmonics( const mxDouble bandwidth, mxComplexDouble *fhat )
{
  int n,k,u;
  ...
  {
    u = 2*n+1;
    factor = sqrt(u)/(2*sqrt(pi));
    for (k=0; k<u; k++)
    {
      fhat[0].real = fhat[0].real*factor;
      fhat[0].imag = fhat[0].imag*factor;
      fhat ++;
    }
  }
}