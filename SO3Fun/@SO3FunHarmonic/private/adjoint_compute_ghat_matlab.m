function fhat = adjoint_compute_ghat_matlab(N,A,varargin)
...
ghat(:,2:end,:) = A(:,N+2:end,:) + (-1).^(k(:,2:end,:)+l(:,2:end,:)) .*A(:,N:-1:1,:);
...
for n=0:N
  ...
  fhat_n = sum(d(:,n+1:end).*permute(d(n+1:end,n+1:end),[3,2,1]).*ghat((-n:n)+N+1,1:n+1,(0:n)+N+1),2);
  % normalize
  fhat_n = sqrt(2*n+1)*fhat_n;
  fhat_n = permute(fhat_n,[1,3,2]);
  A = [flip(flip(fhat_n(:,2:end),1),2),fhat_n];
