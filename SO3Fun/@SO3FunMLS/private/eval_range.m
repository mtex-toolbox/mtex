...
if SO3F.subsample
  localDist = angle(ori.subSet(ori_id), SO3F.nodes.subSet(grid_id));
  dist = sparse(ori_id, grid_id, localDist, N, numel(SO3F.nodes));
end
...
if ~SO3F.centered
  ...
  basis_in_ori = eval_basis_functions(SO3F, orientation.id);
  eval_vector = basis_in_ori.';
end
...
elseif wantConds
  ...
  c_book = solve_lsq_book_varsize( ...
    weights, G, f, nn, solve_args{:});
end
clear weights G f solve_args;


% evaluate the local coefficient vectors
vals(J_idx,:) = permute(sum(basis_in_ori .* ...
