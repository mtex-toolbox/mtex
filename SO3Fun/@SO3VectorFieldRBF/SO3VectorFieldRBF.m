classdef (InferiorClasses = {?SO3FunBingham,?SO3FunCBF,?SO3FunComposition, ...
    ...
    ?SO3VectorFieldHandle,?vector3d}) SO3VectorFieldRBF < SO3VectorField
...
methods
  ...
  function SO3VF = SO3VectorFieldRBF(SO3F, varargin)
    ...
    if isa(SO3F,'SO3Fun') || (isa(SO3F,'function_handle') && isnumeric(SO3F(rotation.id)))
      SO3F = SO3FunRBF(SO3F,varargin{:});
    ...
    if isa(SO3F,'SO3VectorFieldRBF')
      SO3VF = SO3VectorFieldRBF(SO3F.SO3F,SO3F.CS,SO3F.SS,SO3F.internTangentSpace);
      return
    end
    
    if isa(SO3F,'SO3VectorField') || (isa(SO3F,'function_handle') && isa(SO3F(rotation.id),'vector3d'))
