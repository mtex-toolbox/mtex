...
% 
% Here $|N(i,j)|$ denotes the number of all neighboring pixels taking into
% account and $\omega(o_{i,j}, o_{k,l})$ the disorientation angle between
% the orientation $o_{ij}$ in the center and the neighbouring orientation
% $(o_{k,l})$. The specific choice of the set $N(i,j)$ of neighboring
% pixels is crucial for the compution of the KAM. Most commonly the
% following additional constrains are made
%
...
% disorientation angle in radiant. Hence, we dividing by the constant
...
plot(ebsd,ebsd.KAM('threshold',2.5*degree,'order',5) ./ degree)
caxis([0,2])
mtexColorbar
hold on
plot(grains.boundary,'lineWidth',2)
...
cs = crystalSymmetry;
ebsd = EBSDhex(rotation.nan(5,5),N,0:4,{cs,cs,cs,cs,cs},10,1,1);
