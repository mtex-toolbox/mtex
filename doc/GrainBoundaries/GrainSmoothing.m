...
% Proper smoothing has an influence on measures such as total grain boundary
% length, grain boundary curvature, triple point angles or grain boundary
% directions among others.
 
% Wile we used <grain2d.smooth.html |grains.smooth|> before, here we will 
...
plot(grains.boundary('i','i'),'linewidth',2,'linecolor','Fuchsia')
axis([25 100 150 200])
% smooth the grains with default paramters
grains_smooth = smooth(grains);
 
hold on
plot(grains_smooth.boundary('i','i'),'linewidth',2)
hold off
axis([25 100 150 200])
...
sum(grains.boundary('i','i').segLength)
sum(grains_smooth.boundary('i','i').segLength)
 
% If we look at the frequnecy distribution of grain boundary segments, we
% find that some angle are over-represented which is due to the fact that
% without any additional input argument, <grain2d.smooth.html |grains.smooth|>
% only performs just a single iteration

histogram(grains_smooth.boundary('i','i').direction, ...
          'weights',grains_smooth.boundary('i','i').direction.norm,180)
...
for i=1:length(d)
    subplot(1,length(iter),i)
   histogram(d{i}, ...
          'weights',norm(d{i}),180)
end
