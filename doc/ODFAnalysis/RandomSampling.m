...
% uniform background. The fixed random-number seed makes the figures and
% printed values repeatable when this page is published. Omit or change the
% seed when independent realizations are required.
...
for i = 1:length(M)
  ...
  for j = 1:nRep
    ...
    [hwRand(i,j),eRand(i,j)] = ...
      fminbnd(err,2.5,30,optimset('TolX',0.1));
  end
...
%%
% Both curves fall as the sample grows, but the optimized points lie below
% the random ones. Their exponents are similar, so most of the difference
% is a nearly constant factor: an optimized sample supports a distinctly
% sharper kernel at the same size.
...
%%
% Unlike the best halfwidths, the two errors do not differ by only a
% constant factor. The optimized sample has the steeper fitted slope and
% therefore converges faster over this tested range.
%
