...
% define fibres ->  resolution × max(length(h),length(r))
...
% get vertices of the tetrahegons -> length(ori) × 4 matrix with indices
vertices = DSO3.tetra(tetra(:),:);

% set up evaluation matrix -> resolution × max(length(h),length(r)) x
% center
% orientations
...
% reshape -> max(length(h),length(r)) × length(center)
