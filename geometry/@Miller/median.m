function [m,hkl] = median(hkl,varargin)
% median of a list of Miller, principle axes and moments of inertia
%
% Syntax
%   [m, hkl] = median(hkl)
%   [m, hkl] = median(hkl,'robust')
%   [m, hkl] = median(hkl,'weights',weights)
%
% Input
%  hkl      - list of @Miller
%
...
elseif check_option(varargin,'noSymmetry')
  m = median@vector3d(hkl,varargin{:});
  return
end

% get a first guess of the median
...
% compute for each point the median square distance to all vectors
d = median(angle_outer(hkl,plan.r).^2);
