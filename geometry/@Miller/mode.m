function [m,hkl] = mode(hkl,varargin)
% mode of a list of Miller, principle axes and moments of inertia
%
% Syntax
%   [m, hkl] = mode(hkl)
%   [m, hkl] = mode(hkl,'robust')
%   [m, hkl] = mode(hkl,'weights',weights)
%
% Input
%  hkl      - list of @Miller
%
...
elseif check_option(varargin,'noSymmetry')
  m = mode@vector3d(hkl,varargin{:});
  return
end

% get a first guess of the mode
...
% compute for each point the mode square distance to all vectors
d = mode(angle_outer(hkl,plan.r).^2);
