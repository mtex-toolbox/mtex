...
for m = 0:sF.bandwidth+1
  if 0 <= m-1
    fhat_theta(m^2+1:(m+1)^2) = (m-1)*sqrt((m^2-(-m:m)'.^2)/((2*m-1)*(2*m+1))).*[0; sF.fhat((m-1)^2+1:m^2); 0];
  end
  if m+1 <= sF.bandwidth
...
for m = 0:sF_theta.bandwidth+1
  if 0 <= m-1
    fhat_theta_theta(m^2+1:(m+1)^2) = (m-1)*sqrt((m^2-(-m:m)'.^2)/((2*m-1)*(2*m+1))).*[0; sF_theta.fhat((m-1)^2+1:m^2); 0];
  end
