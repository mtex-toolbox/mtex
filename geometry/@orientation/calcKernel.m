...
switch lower(method)
  ...
  case 'magicrule'
    ...
    psi = SO3deLaValleePoussin(kappa,varargin{:});
    return
  ...
  case 'ruleofthumb'
    ...
    psi = SO3deLaValleePoussin(fak*psi.kappa);
...
for k = 1:10
  psi{k} = SO3deLaValleePoussin('halfwidth',30*degree/2^(k/3)); %#ok<AGROW>
end
...
[~,i] = max(c);
psi = SO3deLaValleePoussin(fak*psi{i(1)}.kappa);
