...
if ~all(sum(p2-p1,2)), error('P1 cannot equal P2'), end
...
if nargin<3, [c,ls,lw]=LocalValidateCLSW;% default color, linestyle/width
  ...
  if c=='x'                              % random named color (less white)
    [ignore,i]=sort(rand(1,23)); c=cn(i,:);        %#ok ignore not used
...
if xys, sf=1;
  ...
  sf=norm([xr(2)-xr(1),yr(2)-yr(1),zr(2)-zr(1)])/72;
end
...
if flag, map=get(fig,'colormap');                  % update colors
  ...
  else warning('Arrow3:Magnitude',MagnitudeWarning)
  end
end
set(ax,'ColorOrder',c);                            % update surfaces
ColorOrder=[];
H=arrow3(p1,p2,'o0',w,h,ip,a); H=[H1(:);H(:)];
set(ax,'nextplot','replace');
...
%
% Default ColorOrder Property Colors (bersvhd)
