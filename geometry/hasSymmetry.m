...
% whether the frame a direction is given in carries a point group
%
...
% |unique|, |dot| and |mean| work up to symmetry. The group on the frame
% decides it, so a specimen frame carrying a sample symmetry counts exactly
% as a crystal frame does (ADR 0008).
%
% Syntax
%   out = hasSymmetry(v)
%
...
out = isa(v,'vector3d') && ~isempty(v.frame) && v.frame.id ~= 1;
