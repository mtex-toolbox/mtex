...
close all
plotpdf(get(childs(1),'EBSD'),Miller(1,1,1),...
  'marker','x','markersize',5,'antipodal')
hold on
plotpdf(get(childs(2),'EBSD'),Miller(1,1,1),...
  'marker','x','markersize',5,'antipodal')
...
close all
plotpdf(get(parent,'EBSD'),Miller(0,0,1),'antipodal');
hold on

c = {'y','m','r','c','g'};
for k=1:numel(childs)
  plotpdf(get(childs(k),'EBSD'),Miller(0,0,1),...
