...
if mtexFig.keepAspectRatio
  ...
  lx = @(nc,nr) ceil(min(...
    (figSize(1)-(nc-1)*mtexFig.innerPlotSpacing - nc*sum(mtexFig.tightInset([1,3])))/nc,...
    (figSize(2)-(nr-1)*mtexFig.innerPlotSpacing - nr*sum(mtexFig.tightInset([2,4])))/axesRatio/nr));
