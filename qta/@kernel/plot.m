...
for i = 1:length(kk)
  switch upper(options)
    case 'K'
			...
			optionplot(omega,kk(i).K(cos(omega/2)),varargin{:},'color',colororder(i),'LineWidth',2);
			set(gcf,'Name',['kernel ',inputname(1),' on SO(3)']);
			xlim([-pi,pi]);
		case {'RK','RADON'}
			omega = linspace(-pi,pi,1000);
			optionplot(omega,max(0,kk(i).RK(cos(omega))),varargin{:},'color',colororder(i),'LineWidth',2);
...
%			plot(A,charorder(i),'color',colororder(i),'MarkerSize',10)
    ...
    case 'FOURIER_LOGLOG'
      ...
      set(gcf,'Name',['Fourier coefficients of the kernel ',inputname(1)]);
		case 'EVEN'
			omega = linspace(-pi,pi,1000);
			optionplot(omega,GK(kk(i),cos(omega/2)),varargin{:},'color',colororder(i),'LineWidth',2);
      set(gcf,'Name',['Even part of the kernel ',inputname(1)]);
		case 'ODD'
			omega = linspace(-pi,pi,1000);
			optionplot(omega,UK(kk(i),cos(omega/2)),varargin{:},'color',colororder(i),'LineWidth',2);
      set(gcf,'Name',['Odd part of the kernel ',inputname(1)]);
		case 'EVEN_ODD'
			omega = linspace(-pi,pi,1000);
			optionplot(omega,GK(kk(i),cos(omega/2))-UK(kk(i),cos(omega/2)),varargin{:},'color',colororder(i));
