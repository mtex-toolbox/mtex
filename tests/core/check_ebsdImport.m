...
assert(contains(str,"plot(ebsd({'O''Brien phase','dominant'}),'ipfz')"), ...
  'check_ebsdImport: generated script did not plot the shown phases by name')
str2 = buildImportWizardScript(ebsd,filePath,mapConvention,eulerConvention, ...
  1,[false false true],["not' indexed" "O'Brien phase" "dominant"]);
assert(contains(str2,"plot(ebsd('dominant'),'ipfz')"), ...
  'check_ebsdImport: generated script did not plot a hidden-phase map by name')
