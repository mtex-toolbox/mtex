...
%
% Conventions are compared as objects (== is alignment equality on the
% value class), never through char: every convention that is not axis
% aligned prints 'xyz', so a char comparison would hold vacuously for any
% two of them - it is not an identity.
...
dflt = tensor(M,'rank',2).how2plot;
assert(dflt ~= pC, ...
  ['check_plottingConventionOwnership: the test convention equals the ' ...
  'default one, so it cannot detect a leak - pick another one'])

% (1) nothing else may see it, neither a later tensor nor the shared default
T = tensor(M,'rank',2);
...
% a tensor has one frame, so its reference system is that frame and states
% the same convention - ADR 0008
assert(T.CS == T.frame && T.CS.how2plot == pC, ...
...
assert(sF.frame ~= specimenFrame.default && ...
  specimenFrame.default.how2plot == dflt, ...
...
assert(S2FunHarmonicSym(plain,cs).how2plot == cs.how2plot, ...
...
assert(S2FunHarmonicSym(sF,cs).how2plot == cs.how2plot, ...
...
assert(T.directionalMagnitude.how2plot == pC, ...
...
assert(~isempty(sP) && sP.proj.pC == pC, ...
...
assert(ori.CS.how2plot == pC && ori.SS.how2plot == pC, ...
