...
% column correspondence of the three argument form. A 3 × N matrix holds one
% vector per column and gives a 1 × N list; an N × 3 matrix holds one per row
% and gives an N × 1 list, which is what vector3d.byXYZ has always done.
% Previously anything that was not already 3 × N was transposed into it, so
% an N × 3 matrix came back as a ROW of N vectors - the same coordinates, the
...
% 3 × N gives a row, N × 3 gives a column, and both hold the same vectors
...
assert(isequal(size(byCol),[1 n]), ...
  'vector3d of a 3 × %d matrix is %s, expected [1 %d]', ...
  n, mat2str(size(byCol)), n);

assert(isequal(size(byRow),[n 1]), ...
  'vector3d of a %d × 3 matrix is %s, expected [%d 1]', ...
...
% and the N × 3 reading agrees with byXYZ, which always used it
dev = max(norm(byRow - vector3d.byXYZ(xyz.')));
assert(dev == 0, 'vector3d(N × 3) and byXYZ(N × 3) disagree by %g', dev);
...
% N × 3 with N = 0 is an empty column, not an error
assert(isequal(size(vector3d(zeros(0,3))),[0 1]), ...
  'vector3d of a 0 × 3 matrix is %s, expected [0 1]', ...
...
% MTEX's own code must never reach the 3 × 3 warning
%
% The three crystal axes are a 3 × 3 matrix, so calcAxis built one on every
...
% 'mineral' is what routes through calcAxis's alignment branch, which built the 3 × 3
...
% 3 × 3 satisfies both readings, and a matrix that is neither is rejected
...
assert(strcmp(id,'MTEX:vector3d:ambiguousMatrix'), ...
  'a 3 × 3 matrix raised %s instead of MTEX:vector3d:ambiguousMatrix', id);

% ... and is read column wise, i.e. as the 3 × N case
assert(isequal(size(v),[1 3]), ...
  'the ambiguous 3 × 3 matrix gave %s, expected the column wise [1 3]', ...
  mat2str(size(v)));

try
  vector3d(rand(4,5));
  error('check_vector3d:noError','a 4 × 5 matrix of coordinates was accepted');
catch ME
  assert(strcmp(ME.identifier,'MTEX:vector3d:wrongSize'), ...
    'a 4 × 5 matrix raised %s instead of MTEX:vector3d:wrongSize', ME.identifier);
end
...
% =========================================================================
function checkByXYZ
% byXYZ takes N × 3 and N × 2, including empty, and rejects the rest
%
% The N × 2 branch used to pass the SCALAR 0 as the z coordinate and rely on
% the constructor repmat-ing it to the size of x and y. For an empty input
% there is no non singular size to repmat to, so z stayed 1 × 1 against a
% 0 × 1 x and y and the constructor threw "Coordinates have different size".
% grain2d/checkInside walked into it - it appended a zero column to a query
% that was already n × 3, so every call took the non-three-column branch,
...
for n = [0 1 5]
  ...
  assert(isequal(size(v),[n 1]), ...
    'byXYZ of a %d × 2 gave %s, expected [%d 1]', n, mat2str(size(v)), n);
  assert(all(v.z(:) == 0), 'byXYZ of a %d × 2 did not set z to zero', n);

  v = vector3d.byXYZ(zeros(n,3));
  assert(isequal(size(v),[n 1]), ...
    'byXYZ of a %d × 3 gave %s, expected [%d 1]', n, mat2str(size(v)), n);

end

% [] carries no width - an evaluation over no nodes returns one
v = vector3d.byXYZ([]);
assert(isempty(v), 'byXYZ of [] gave %s, expected an empty', mat2str(size(v)));
