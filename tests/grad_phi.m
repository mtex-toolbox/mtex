...
delta = 0.05 * degree;
ori = orientation('Euler',phi1 + [-1 1] * delta,Phi,phi2,ori.CS,'nfft');

g_phi1 = diff(odf.eval(ori))/delta/2;

ori = orientation('Euler',phi1,Phi + [-1 1] * delta,phi2,ori.CS,'nfft');

g_Phi = diff(odf.eval(ori))/delta/2;

ori = orientation('Euler',phi1,Phi,phi2 + [-1 1] * delta,ori.CS,'nfft');
