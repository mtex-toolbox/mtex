...
function Plot_DL23_Range3(A, B, C, lw1)
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
    y = linspace(0,((1 - sqrt(2*r^2 - 1))/2),100);x = y - y;z = sqrt(r^2 - y.^2);if(abs(max(imag(z))) < 1e-05); z = real(z); end
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);

    x = linspace(0,((1 - sqrt(2*r^2 - 1))/2),100);y = x - x;z = sqrt(r^2 - x.^2); if(abs(max(imag(z))) < 1e-05); z = real(z); end
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);


    temp1phi = linspace(0,pi/2,100); tempvar1 = cos(temp1phi) + sin(temp1phi); 
