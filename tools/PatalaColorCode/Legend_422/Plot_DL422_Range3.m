...
function Plot_DL422_Range3(A, B, C, lw1)
    ...
    bv = rodri2volpreserv(b); ba = areapreservingz(bv); S = colormap422(b);
    
    theta1 = acos((tan(pi/8))/r); theta2 = asin(1/r);
    t1 = [r*sin(theta1) 0 r*cos(theta1)]; 
    t2 = [r*sin(theta1)*cos(pi/4) r*sin(theta1)*sin(pi/4) r*cos(theta1)];
    t3 = [r*sin(theta2) 0 r*cos(theta2)]; 
    t4 = [r*sin(theta2)*cos(pi/4) r*sin(theta2)*sin(pi/4) r*cos(theta2)]; 
    t5 = [r*cos(acos(1/r)) r*sin(acos(1/r)) 0]; 
    t6 = [r*cos(asin(1/r) - pi/4) r*sin(asin(1/r) -pi/4) 0];
    ...
    y = linspace(r*sin(acos(1/r)),r*sin(asin(1/r)-pi/4),100);z = y - y;x = sqrt(r^2 - y.^2); 
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); 
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);

    z = linspace(tan(pi/8),r*cos(theta2),100);y = x - x;x = sqrt(r^2 - z.^2); 
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); 
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);

    z = linspace(tan(pi/8),r*cos(theta2),100);x = cos(pi/4)*sqrt(r^2 - z.^2); y=tan(pi/4)*x;
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv);
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
    ...
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv);
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
    temppts = [x' y' z']; temppts = [temppts(:,1)-1 temppts(:,2)-tan(pi/8) temppts(:,3)];
    g = [cos(-5*pi/4) -sin(-5*pi/4) 0; sin(-5*pi/4) cos(-5*pi/4) 0; 0 0 1];
    temppts = temppts*g; temppts = [temppts(:,1)+1 temppts(:,2)+tan(pi/8) temppts(:,3)];
    ptsv = rodri2volpreserv(temppts);ptsa = areapreservingz(ptsv); 
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);

    y = linspace(0,sqrt(r^2 - (tan(pi/8))^2)*sin(pi/4),100);z = (tan(pi/8))*ones(1,size(y,2));
    x = sqrt(r^2 - y.^2 - z.^2); 
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); 
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
