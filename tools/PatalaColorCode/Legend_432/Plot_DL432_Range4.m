...
function Plot_DL432_Range4(A, B, C, lw1)
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
    ...
    tempd1 = sqrt(2) - 1; theta1 = acos(((1-tempd1)- sqrt(2*(tan(alphaAng)^2 - tempd1^2) - (1-tempd1)^2))/(2*tan(alphaAng)));
    theta2 = acos((1-sqrt(6*(tan(alphaAng))^2 -2))/(3*tan(alphaAng))); theta3 = acos((sqrt(tan(alphaAng)^2 - 2*(tempd1^2)))/(tan(alphaAng)));
    z = linspace(r*cos(theta2),r*cos(theta3),100); x = sqrt((r^2 - (z.^2))/2); y = x;
    ptsv = rodri2volpreserv([x' y' z']);
    ptsa = areapreservingx(ptsv); plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
    theta = linspace(theta1,theta2,100); tempd3 = tan(alphaAng)*cos(theta); phi = acos((((1-tempd3)+(sqrt(2*((tan(alphaAng))^2 - tempd3.^2)-(1-tempd3).^2)))/2)./(tan(alphaAng)*sin(theta)));
    z = r*cos(theta); x = r*sin(theta).*cos(phi); y = sqrt(r^2 - x.^2 - z.^2); 
    ptsv = rodri2volpreserv([x' y' z']);
    ptsa = real(areapreservingx(ptsv)); plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
    z = linspace(r*cos(theta1),r*cos(theta3),100); x = (sqrt(2)-1)*ones(size(z)); y = sqrt(r^2 - x.^2 - z.^2); 
    ptsv = rodri2volpreserv([x' y' z']);
    ptsa = areapreservingx(ptsv); plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
