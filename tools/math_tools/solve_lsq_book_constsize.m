...
Q_good = get_option(varargin, {'Qgood','Q_good','Q good'}, 2.5); % or 1
Q_bad  = get_option(varargin, {'Qbad', 'Q_bad', 'Q bad' }, 2.5); % or 6
...
alpha_min = get_option(varargin, {'alphamin','alpha_min','alpha min'}, 0);
...
mineigs = max(reshape(min(eigs,[], 1), [], 1), realmin);
...
Gram_book(diag_idx) = Gram_book(diag_idx) + diag_offsets(:);
