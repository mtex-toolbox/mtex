...
%
% Note
%  The data is returned in the plotting convention of the session. mtexdata
%  used to impose 'y↑→x' on ten of the datasets, which made the alignment of
%  a documentation page invisible and overrode whatever the reader had set.
%  A page that wants a particular alignment now says so itself:
%
    ...
    case 'EBSD'
      switch lower(name)
        ...
        case 'emsphinx'
          ...
          % an in situ deformed austenitic steel indexed with EMSphInx: 0.4 micron
          % steps and about 0.1 degree noise, so the subgrain structure is resolved
          % - the file names no phase, its master patterns were alpha and gamma iron
