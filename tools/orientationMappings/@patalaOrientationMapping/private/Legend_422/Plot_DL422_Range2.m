...
function Plot_DL422_Range2(A, B, C, lw1)
    ...
    theta1 = acos((tan(pi/8))/r); theta2 = pi/2;
    ...
    y = linspace(0,r*sin(pi/4),100);z = y - y;x = sqrt(r^2 - y.^2); 
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); 
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
    z = linspace(tan(pi/8),0,100);y = x - x;x = sqrt(r^2 - z.^2); 
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); 
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
    z = linspace(tan(pi/8),0,100);x = cos(pi/4)*sqrt(r^2 - z.^2); y=tan(pi/4)*x;
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv);
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
