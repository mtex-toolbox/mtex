...
function Plot_DL422_Range4(A, B, C, lw1)
    ...
    t2 = [r*sin(theta1)*cos(asin(1/(r*sin(theta1)))-pi/4) ...
        r*sin(theta1)*sin(asin(1/(r*sin(theta1)))-pi/4) r*cos(theta1)];
    t3 = [1 tan(pi/8) r*cos(theta2)];
    ...
    z = linspace((tan(pi/8)),r*cos(theta2),100);x = ones(1,size(z,2)); y=sqrt(r^2 - z.^2-x.^2);
    ...
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
    temp1 = sqrt(r^2 - (tan(pi/8))^2 -1);
    g = [cos(-5*pi/4) -sin(-5*pi/4) 0; sin(-5*pi/4) cos(-5*pi/4) 0; 0 0 1];
    temp2 = [0 temp1-tan(pi/8) 0]*g;
    y = linspace(sqrt(r^2 - (tan(pi/8))^2 -1),...
        temp2(2) + tan(pi/8),100);z = (tan(pi/8))*ones(1,size(y,2));
    x = sqrt(r^2 - y.^2 - z.^2); 
    ptsv = rodri2volpreserv([x' y' z']);ptsa = areapreservingz(ptsv); 
    plot3(ptsa(:,1),ptsa(:,2),2*ones(1,100),'-','Color',[0 0 0],'LineWidth',lw1);
